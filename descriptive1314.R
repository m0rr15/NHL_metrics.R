...
# This script creates variables (section "Variable Treatment") and plays a bit
# around with the retrieved NHL data (section "Descriptive Statistics"). In the 
# "Statistics" section it computes the probailities and marginal probabilites of
# winning a game in function of the Goals scored For and Against. Put
# differently, it reckons the value of the n'th goal scored.
...
# TIDY dataframes!!
p_s.for.dat  <- melt(p_s.for.dat,id.vars=c("awayscore","homescore"),
...
ggplot(p_s.for.dat, aes(x=awayscore,
                             y=Probability, colour=Location)) +
  geom_line() +
  xlab("Goals scored") +
  ylab("Probability") +
  ggtitle("Probability to win 2 points versus Goals Scored") +
  scale_x_continuous(breaks=seq(0, 10, 1))  # Ticks from 0-10, every 1
ggplot(p_s.for.dat.diff, aes(x=Score,
                                  y=mProbability, colour=Location)) +
  geom_line() +
  xlab("Goals scored") +
...
# TIDY DF!!!
p_s.ag.dat <- melt(p_s.ag.dat,id.vars=c("awayscore","homescore"),
                     variable.name="Location",value.name="Probability")
p_s.ag.dat.diff <- melt(p_s.ag.dat.diff,id.vars=c("Score"),
                          variable.name="Location",value.name="mProbability")
